...
def test_rubric_failure_names_what_is_missing():
    broken = Path(tempfile.mkstemp(suffix=".md")[1])
    broken.write_text("# x\n\n## 1. Only one\n\n### GREEN\nfine\n\n### RED\nbad\n")
    try:
        cycle.rubric(broken)
...
def test_stored_total_that_disagrees_is_reported_not_corrected():
    ...
    assert row["stored_total"] == "13"                          # the file's claim, kept
...
def test_first_save_creates_the_file_with_the_template_header():
    ...
    try:
        ...
        assert not (d / "cycle-score.csv.tmp").exists(), "atomic write left its temp file"
    finally:
        for f in d.iterdir():
            f.unlink()
        d.rmdir()
...
def test_build_cycle_reports_latest_and_previous():
    ...
    try:
        out = cycle.build_cycle(p, today=date(2026, 9, 1))
    finally:
        p.unlink()
