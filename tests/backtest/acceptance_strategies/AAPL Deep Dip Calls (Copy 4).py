...
class AAPLDeepDipCalls(Strategy):
    ...
    --------------------
    Buys 2-year, ~20% out-of-the-money call options on GOOG after a 25% decline from the running high since the bot started.
    ...
    This code was refined based on the user prompt: 'Now modify this Algo to sell the Appl call after holding it for 1 year and not when the stock recovers by 25% and everytime you buy it after a 25% drawdown, sell the call option/LEAP 1 year after'
    """
    ...
    def initialize(self):
        ...
        # Helper to discover expirations/strikes and execute multi-leg orders if needed later
    ...
    def on_trading_iteration(self):
        ...
        if open_pos is None:
            ...
            # DEBUG: Log ATH tracking around key dates (2022 drawdown, July split)
            current_date = self.get_datetime().date()
            if current_date.year == 2022 and current_date.month in [1, 7, 10, 11]:
                self.log_message(
    ...
    def _attempt_buy_option(self, underlying: Asset, stock_price: float, otm_pct: float, max_spread_pct: float):
        """Select a ~20% OTM call with ~2y expiry and place a limit buy using the mid price."""
        params = self.get_parameters()
        symbol = params["symbol"]
        ...
        expiry_date = self.options_helper.get_expiration_on_or_after_date(target_exp_dt, chains, "call")
        ...
        # Build the option asset we intend to trade
        option_asset = Asset(
            symbol,
        ...
        # Try bid/ask for mid; fall back to last price if quotes are not available
        opt_quote = self.get_quote(option_asset)
        if opt_quote is not None and opt_quote.ask is not None and opt_quote.bid is not None and opt_quote.ask > 0:
            spread = opt_quote.ask - opt_quote.bid
            spread_pct = spread / opt_quote.ask if opt_quote.ask else None
            if spread_pct is not None and spread_pct > max_spread_pct:
                self.log_message(
                    f"Skipping illiquid option (spread {spread_pct:.0%} > {max_spread_pct:.0%}).", color="yellow"
                )
                return
            mid_price = opt_quote.mid_price
        else:
            mid_price = self.get_last_price(option_asset)

        if mid_price is None or mid_price <= 0:
            self.log_message("No valid price for the chosen option; skipping buy.", color="yellow")
            return

        # Position sizing: spend up to the risk_per_trade portion of cash, rounded to whole contracts
        cash = self.get_cash()
        contracts = int(math.floor((cash * float(risk_per_trade)) / (float(mid_price) * 100.0)))
        if max_contracts is not None:
            contracts = min(contracts, max_contracts)
        if contracts < 1:
            self.log_message(
                f"Insufficient cash for even 1 contract at ~${mid_price:.2f} (x100). Holding cash.", color="yellow"
            )
            return

        # Place a limit BUY at the mid price to avoid overpaying
        order = self.create_order(option_asset, contracts, Order.OrderSide.BUY, limit_price=float(mid_price))
        submitted = self.submit_order(order)
        if submitted is not None:
            self.log_message(
                f"Submitted BUY {contracts}x {symbol} {expiry_str} {chosen_strike:.2f}C at ~${mid_price:.2f} (limit)",
                color="green",
            )
        else:
...
if __name__ == "__main__":
    ...
    if IS_BACKTESTING:
        ...
        results = AAPLDeepDipCalls.backtest(
            PolygonDataBacktesting,  # Correct signature: pass datasource_class positionally
