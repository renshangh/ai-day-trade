"""Profile DataBento with Polars-native MES Momentum SMA-9 strategy over 2-3 days.

This is a polars-native version that uses return_polars=True and polars DataFrame operations
for maximum performance. Comparison with the pandas version shows the full optimization potential.
"""
...
class MESMomentumSMA9PolarsNative(Strategy):
    """Polars-native MES trading strategy with SMA-9 gate and ATR-based risk management.

    This version uses return_polars=True and polars DataFrame operations throughout
    for maximum performance. All pandas operations have been replaced with polars equivalents.
    """
    ...
    def _compute_indicators_polars(self, df: pl.DataFrame, last_price: float, sma_period: int, atr_period: int) -> tuple:
        ...
        last_row = df.tail(1)
        sma_latest = last_row["sma"][0] if last_row["sma"][0] is not None else None
        atr_latest = last_row["atr"][0] if last_row["atr"][0] is not None else None

        # Blend SMA with latest price (live calculation)
        if last_price is not None and len(df) >= sma_period:
            # Get last (sma_period - 1) closes and append current price
            closes = df.tail(sma_period - 1)["close"].to_list()
    ...
    def on_trading_iteration(self):
        ...
        # CRITICAL: Get historical prices with return_polars=True for maximum performance
        ...
        if last_price is None:
            # Polars way to get last close
            last_price = df.tail(1)["close"][0]
        self.vars.last_price = float(last_price)

        # Compute indicators using polars operations
        sma_live, atr_val = self._compute_indicators_polars(df, last_price, params["sma_period"], params["atr_period"])
...
def run_polars_native_profile() -> float:
    """Run Polars-native MES Momentum strategy backtest and profile it."""
    label = "mes_momentum_polars_native"
    ...
    # Run backtest with Polars data source
